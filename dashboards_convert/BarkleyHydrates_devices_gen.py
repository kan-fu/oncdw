...
location_code = "BACHY"
location_name = "Barkley Canyon Hydrates"
html_filename = "BarkleyHydrates_devices"
json_filename = "Barkley_Hydrates_Devices"
...
for device, sensor in zip(devices, sensors):
    device_id, device_name = re.match(r"(\d+ & \d+)(.*)", device).groups()
    device_id1, device_id2 = device_id.split(" & ")
    sensor_id1, sensorid2, sensor_name = re.match(r"(\d+),(\d+)(.*)", sensor).groups()
    res1.append(
        {
            "deviceId": device_id,
            "deviceName": device_name,
            "sensors": [
                ((int(sensor_id1), sensor_name), (int(sensorid2), sensor_name))
            ],
            "locationCode": location_code,
            "locationName": location_name,
...
json.dump(res1, open(f"./pages/{json_filename}_1.json", "w"), indent=2)
json.dump(res2, open(f"./pages/{json_filename}_2.json", "w"), indent=2)
