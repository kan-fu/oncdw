...
def get_date_from_last_days(
    last_days: int, date_to=datetime.now(timezone.utc)
) -> tuple[str, str]:
    """
    ...
    date_to : datetime, optional
        dateTo used in the web service. Default is the current datetime in the UTC timezone.
...
def _get_id_from_sensor(sensor: int | dict | tuple | list | str) -> int:
    ...
    elif isinstance(sensor, tuple) or isinstance(sensor, list):
